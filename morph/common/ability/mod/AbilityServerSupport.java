...
public class AbilityServerSupport {
	
	private HashMap<String, String[]> customMobAbilityMapping = new HashMap<String, String[]>();

	public static AbilityServerSupport build(String src){
		AbilityServerSupport instance;
		Gson gson = new Gson();
		try{
			Reader fileIn = new InputStreamReader(new ByteArrayInputStream(src.getBytes("UTF-8")));
			instance = gson.fromJson(fileIn, AbilityServerSupport.class);
		}catch(Exception e){
			e.printStackTrace();
			instance = new AbilityServerSupport();
		}
